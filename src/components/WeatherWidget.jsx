...
export default function WeatherWidget() {
  const [weather, setWeather] = useState({
    ...
    condition: '--',
    rainProb: 0,
    recommendation: 'Cargando datos meteorológicos...',
    loading: true,
    tipoAlerta: 'ideal'
  });
  ...
  useEffect(() => {
    async function fetchWeather() {
      try {
        // Coordenadas para Barinas, Venezuela (Sector aproximado)
        const lat = 8.6226;
        const lon = -70.2075;
        const res = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current_weather=true&daily=precipitation_probability_max,temperature_2m_max&timezone=America/Caracas`);
        ...
        if (data && data.current_weather && data.daily) {
          ...
          const tempMax = data.daily.temperature_2m_max[0] || tempActual;

          // Mapeo simple de condiciones meteorológicas
          let condition = 'Despejado';
          if (code >= 1 && code <= 3) condition = 'Parcialmente nublado';
          if (code >= 45 && code <= 48) condition = 'Niebla';
          if (code >= 51 && code <= 67) condition = 'Llovizna / Lluvia ligera';
          if (code >= 71 && code <= 77) condition = 'Nieve';
          if (code >= 80 && code <= 82) condition = 'Lluvia fuerte / Aguaceros';
          if (code >= 95) condition = 'Tormenta eléctrica';

          // Lógica de Alertas para Siembra y Riego
          let tipoAlerta = 'ideal';
          let decision = 'Clima ideal. Continuar con el plan de siembra y fertilización regular.';

          if (rainProb > 60 || (code >= 61 && code <= 67) || code >= 80) {
            tipoAlerta = 'lluvia';
            decision = 'Alta probabilidad de lluvia. Postergar fertilización foliar y asegurar drenajes en lotes bajos.';
          } else if (tempMax > 33 && rainProb < 20) {
            tipoAlerta = 'sequia';
            decision = `Alerta de calor excesivo (${tempMax}°C). Aumentar frecuencia de riego profundo en las mañanas.`;
          }
        ...
        setWeather(prev => ({
          ...prev,
          loading: false,
          recommendation: 'Error al conectar con el servidor meteorológico.'
        }));
      }
    }
  ...
  return (
    <div className={styles.container}>
      <div className={styles.card}>
        <svg fill="none" viewBox="0 0 342 175" height="175" width="342" xmlns="http://www.w3.org/2000/svg" className={styles.background}>
        ...
        <p className={styles.mainText}>{weather.temp}°</p>
        
        <div className={styles.info}>
          <div className={styles.infoLeft}>
            <p className={styles.textGray}>Probabilidad: {weather.rainProb}%</p>
            <p style={{ margin: 0 }}>Barinas, La Yuca</p>
          </div>
          <p className={styles.infoRight}>{weather.condition}</p>
        </div>
      </div>
      ...
      <div className={styles.alertBox} style={{
        ...
        <span className={styles.alertIcon}>
          {weather.tipoAlerta === 'lluvia' ? '🌧️' : weather.tipoAlerta === 'sequia' ? '☀️' : '🌤️'} Logística de Campo:
        </span>
